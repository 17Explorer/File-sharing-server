 ...
 */
 /* -------------------------------------------*/
int check_user_pwd( char *username, char *pwd )//负责读取密码
{
    ...
    process_result_one(conn, sql_cmd, tmp); //执行sql语句，结果集保存在tmp，读取到的密码
    if(strcmp(tmp, pwd) == 0)//strcmp为字符串比较函数，若两字符串相等，返回0
    {
        ret = 0;//密码正确
    }
    else
...
int set_token(char *username, char *token)
{
    ...
    base64_encode((const unsigned char *)enc_tmp, enc_len, base64);  //base64编码 防止不可见字符，因为有些字符看不见
